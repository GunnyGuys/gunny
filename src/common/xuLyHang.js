...
function findSoKieuHang(value) {
  let temp = normalizeString(value);
  const arr = ["hang", "hag", "ha", "h"];
  let count = 0;
  arr.forEach((item) => {
...
function xuLyHang(message) {
  const messageLen = message.length;
  const arr = ["hang", "hag", "ha", "h"];

  /// Lấy ra các mẫu có thể là số dạng hàng
  const patterns = [];
  arr.forEach((item) => {
    for (let i = 1; i < 10; i++) {
  ...
  /// Lấy ra vị trí đầu và cuối của một chuỗi con có thể là số dạng hàng
  const response = [];
  result.forEach((item) => {
  ...
  response.forEach((item) => {
    ...
    const soHang = findSoKieuHang(value);
    if (soHang !== "") {
